...
class Comparator:
    ...
    def get_best_mapping_greedy(self, graph1: networkx.DiGraph, graph2: networkx.DiGraph):
        ...
        while True:
            ...
            if len(candidates) > 1:
                candidates = \
                    maxima(candidates, key=lambda mapping_element: self.look_ahead(graph1, graph2, mapping_element))[0]
            ...
            for group_key in [value for value in grouped_mapable_adjacent_edges1 if
                ...
                edge_mappings = get_mappings(grouped_mapable_adjacent_edges1[group_key],
                                             grouped_mapable_adjacent_edges2[group_key])
                if len(edge_mappings) == 1:
                    mapping.update(edge_mappings[0])
                else:
                    best_edge_mappings, _ = \
                        maxima(edge_mappings, key=lambda edge_mapping: len(self.get_matches(edge_mapping)))
                    mapping.update(best_edge_mappings[0])
        return mapping, len(self.get_matches(mapping))

    def look_ahead(self, graph1: networkx.DiGraph, graph2: networkx.DiGraph, mapping_element: Tuple[Any, Any]):
        edges1 = get_edges(graph1)
        edges2 = get_edges(graph2)
        outgoing_labeled_transitions1 = \
            get_labeled_edges(graph1, edges1, self.labeled_nodes1, mapping_element[0], True)
        outgoing_labeled_transitions2 = \
            get_labeled_edges(graph2, edges2, self.labeled_nodes1, mapping_element[1], True)
        ...
        incoming_labeled_transitions1 = \
            get_labeled_edges(graph1, edges1, self.labeled_nodes1, mapping_element[0], False)
        incoming_labeled_transitions2 = \
            get_labeled_edges(graph2, edges2, self.labeled_nodes1, mapping_element[1], False)
